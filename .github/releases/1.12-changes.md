...
- **CI and images:** `actions/checkout v7` and `docker/login-action v4` are current. GoReleaser's ko pipeline publishes scratch-based images with CA roots directly to GHCR and Docker Hub for all six existing architectures; QEMU, Buildx image builds, and the deprecated `dockers` / `docker_manifests` pipes are no longer used.
