...
**Date:** 2026-07-02, revised 2026-08-29.
...
- **Toolchain and Go modules:** release binaries use Go 1.27.0; `golang.org/x/crypto 0.55.0`, `golang.org/x/net 0.57.0`, `golang.org/x/sync 0.22.0`, and `github.com/fsnotify/fsnotify 1.10.1` are pinned (indirect `x/sys 0.47.0`, `x/text 0.41.0`). The `x/crypto` update addresses [GO-2026-6303](https://pkg.go.dev/vuln/GO-2026-6303). `go build ./...`, `go vet ./...`, `go mod verify`, the full race suite, and `govulncheck ./...` pass with no reachable vulnerabilities.
- **CI and images:** `actions/checkout v7` and `docker/login-action v4` are current. GoReleaser's ko pipeline publishes scratch-based images with CA roots directly to GHCR and Docker Hub for all six existing architectures. Buildx creates only the local six-platform CA base (without QEMU); the deprecated `dockers` / `docker_manifests` pipes and per-architecture Dockerfile application builds are gone.
...
*Comparison basis: `ai-incoming@e0b639d` vs `master@310eec3` (released v1.11.7). Produced 2026-07-02, revised 2026-08-29.*
